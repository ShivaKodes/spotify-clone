...
const copyBillingDetailsToCustomer=async(uuid:string,payement_method:Stripe.PaymentMethod)=>{

    const customer=payement_method.customer as string

    const {name,phone,address}=payement_method.billing_details;
    ...
    const {error}=await supabaseAdmin.from('users').update({billing_address:{...address},payment_method:{...payement_method[payement_method.type]}}).eq('id',uuid)
...
const manageSubscriptionStatusChange=async(subscriptionId:string,customerId:string,createAction=false)=>{
    ...
    console.log("subscription insted / updated",subscription.id,uuid)
